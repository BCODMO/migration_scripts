...
# File handler
file_handler = logging.FileHandler("filename_finder_helper.log")
...
def generate_change_summary(matches, dataset_id, pipeline):
    ...
    for match in matches:
        ...
        for detailed_diff in pipeline["comparison"]["detailed_differences"]:
            if match == detailed_diff["filename"]:
                changes_detailed = changes_detailed + [
                    {**d, "filename": match} for d in detailed_diff["cell_differences"]
                ]
        ...
        changes_summary += f"""The file {match} from dataset {dataset_id} had {total_cells_impacted} value{'s that were' if total_cells_impacted > 1 else ' that was'} impacted.
...
for orcid in js.keys():
    ...
    for p in (
        js[orcid]["inferred_submission_id_not_in_dataset_pipelines"]
        + js[orcid]["no_submission_id"]
    ):
        ...
        if dataset_id == "877899" or dataset_id == "836121":
            # This is an older version, the new version is dealt with in final.json
            continue
        if dataset_id == "849330":
            # This pipeline never got used
            continue
        if dataset_id == "835589":
            # This is an older version, the new version had an error and will be dealt with manually
            continue
        if dataset_id == "855139":
            # There is some weirdness going on with the versioning/pipeline and will be dealt with manually
            continue
        ...
        problem_files = []
        for fc in pipeline["comparison"]["file_comparisons"]:
            assert fc["etags_match"] or fc.get("cell_differences_count", 0) > 0
            if not fc["etags_match"]:
                problem_files.append(fc["filename"])

        changes_summary, changes_detailed = generate_change_summary(
            problem_files, dataset_id, pipeline
        )
            ...
            "files": [
                {
                    "original_filename": "",
                    "new_filename": "",
                    "fixed_file_path": f"{pipeline["test_prefix"]}/{problem_file}",
